...
def mutar(poblacion):
    for i in range(len(poblacion)-pressure):
            if random.random() <= posibilidadMutacion: #Cada individuo de la poblacion (menos los padres)
                    ...
                    while nuevo_valor == poblacion[i][punto]: #Si el valor de la mutacion es el mismo del punto busca otro
...
def mse(imageA):
    ...
    err = np.sum((np.array(imageA) - np.array(listaGenesMeta)) ** 2)
...
loggear("Fitness poblacion inicial: %s\n"%[calcularAdaptabilidad(poblacion[d]) for d in range(tamanoPoblacion)])
...
loggear("Fitness poblacion final: %s"%[calcularAdaptabilidad(poblacion[d]) for d in range(tamanoPoblacion)])
